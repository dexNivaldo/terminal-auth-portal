            ...
            <div className="mt-6 space-y-3">
              ...
              <div className="flex gap-2 flex-wrap">
                {savedEntries.map((entry) => (
                  <div
                    key={entry.id}
                    className={`group text-nowrap relative ${entry.isProd ? 'border-yellow-500' : 'border-gray-500'} border flex items-center gap-2 bg-slate-800/50 backdrop-blur-sm px-3 py-1.5 rounded-lg cursor-pointer hover:bg-slate-700/50 transition-colors duration-200`}
                    onClick={() => handleTagClick(entry)}
                  >
                    <span className={`text-sm block ${entry.isProd ? 'text-yellow-500' : 'text-gray-300'}`}>
                      {entry.patente} • {entry.terminal.code}
                      <small className="line-clamp-1">
                          {entry.isProd ? `prod | ${entry.role}` : `dev | ${entry.role}`}
                      </small>
                    </span>
                    <button
                      onClick={(e) => {
